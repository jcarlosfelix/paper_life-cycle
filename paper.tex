...
stages that all living beings undergo \cite{read1968system}: being born, growing up, reproducing, and
...
\section{Description} 
% Description of the sections of the Paper
% We structure this research paper as follows

We present the description of the following sections in our research paper.

\begin{description}  % When enumerating with description, bullet alternatives: cdot, bullet, ast
    \item[$\cdot Background$] We do a brief explanation of our motivation for this research.
    \item[$\cdot Proposal$] We present an algorithm inspired by the Life-Cycle of animal species.
    \item[$\cdot Experiments$] We specify our experimental setup, configuration, and results.
    \item[$\cdot Discussion$] We analyze and describe some of our research findings.
    \item[$\cdot Conclusions$] We present some inferences based on our experiments' results.
    \item[$\cdot References$] We enumerate the references for our research paper.
\end{description}
...
general abstraction to identify what those species have in common. Some
questions in our train of thought were:

\begin{description}  % When enumerating with description, bullet alternatives: cdot, bullet, ast
    \item[$\cdot$] How does nature work?
    \item[$\cdot$] Is evolution real?
    \item[$\cdot$] How do we enforce natural selection in evolution?
    \item[$\cdot$] What is the role of the attraction between couples on reproduction?
    \item[$\cdot$] How much of a factor is the attraction's role in the evolution of the species?
    \item[$\cdot$] What is the role of death in all of this?
    \item[$\cdot$] How much of an impact does death have on the evolution of a species?
    \item[$\cdot$] How do we include those ideas into our algorithm?
\end{description}
